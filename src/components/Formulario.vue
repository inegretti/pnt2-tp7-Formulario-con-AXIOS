<template >
  ...
  <hr>
  <h2>Tablas</h2>
  <h3>Actualizacion en el momento</h3>
  <pre v-show="formState.$dirty ||usuarios.length!=0">nombre: {{formData.nombre}} edad:{{formData.edad}} email{{formData.email}}</pre>
...
<script >
  ...
  export default  {
    ...
    methods: {
      async enviar(){
        let obj= {nombre:this.formData.nombre,edad:Number(this.formData.edad),email:this.formData.email}
        console.log(obj)
        await this.axios.post(this.url, {
        name:this.formData.nombre,edad:Number(this.formData.edad),emai:this.formData.email
      });
      await this.axios(this.url).then(respuesta=>{this.usuarios=respuesta.data}).catch(error=>console.error(error));
        //this.usuarios.push({nombre:this.formData.nombre,edad:Number(this.formData.edad),email:this.formData.email})
